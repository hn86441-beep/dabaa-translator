...
app = FastAPI(title="HN Translator API")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_methods=["*"],
...
def notify_whatsapp(message):
    ...
    try:
        ...
        requests.get(
            ...
            params={"phone": WA_PHONE, "text": text, "apikey": CALLMEBOT_APIKEY},
            timeout=8,
        )
    except Exception:
        pass
...
def smart_translate(text, tgt, src="Auto-Detect"):
    ...
    if GROQ_KEY and len(text) <= 1500:
        ...
        if r:
            return r, "AI Contextual"
    # Google fallback
    try:
        res = GoogleTranslator(
            source=LANGS.get(src, {}).get("g", "auto"),
            target=LANGS.get(tgt, {}).get("g", "en"),
        ).translate(text)
        if res:
            return res, "Google"
    except Exception:
...
async def api_translate(req: Request):
    data = await req.json()
    trans, eng = smart_translate(
        data.get("text", ""),
        data.get("tgt", "Arabic"),
        data.get("src", "Auto-Detect"),
    )
...
async def api_ocr(file: UploadFile = File(...)):
    img = await file.read()
    if not GROQ_KEY:
        return JSONResponse({"txt": None, "err": "No Groq Key"})
    try:
        ...
        return JSONResponse({"txt": None, "err": str(e)})
...
async def api_stt(file: UploadFile = File(...), lang: str = Form("auto")):
    ...
    if data:
        return JSONResponse({"txt": data.get("text", "").strip(), "err": None})
...
async def api_srt(file: UploadFile = File(...), tgt: str = Form("Arabic")):
    ...
    if not data:
        return JSONResponse({"blocks": None, "err": err})
    blocks = []
...
async def api_group(file: UploadFile = File(...), tgt: str = Form("Arabic")):
    ...
    data, err = groq_stt(audio, lang="auto", verbose=True)
    if not data:
        return JSONResponse({"results": None, "err": err})
    results = []
...
async def api_extract(file: UploadFile = File(...)):
    ...
    try:
        if ext == ".pdf":
            ...
            return {"txt": None, "err": "Unsupported file type"}
    except Exception as e:
        return {"txt": None, "err": str(e)}
