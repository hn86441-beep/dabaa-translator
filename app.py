...
.block-container {
    padding: 0.4rem 0.3rem !important;   /* هامش صغير جداً */
    max-width: 100% !important;          /* استغلال كامل العرض */
    position: relative;
    z-index: 1;
}
...
div[data-testid="stAudioInput"] > div {
    ...
    border: 1.5px solid rgba(78,203,160,0.3) !important;
...
textarea:focus {
    ...
    box-shadow: 0 0 0 2px rgba(78,203,160,0.1) !important;
}
...
@media (max-width: 480px) {
    ...
    div[data-testid="stAudioInput"] > div { min-width: 100px !important; }
    div[data-testid="stAudioInput"] button { font-size: 10px !important; padding: 0.1rem 0.6rem !important; }
    .stSelectbox > div > div { font-size: 10px !important; min-height: 22px !important; }
    .stButton > button { font-size: 10px !important; min-height: 24px !important; }
    textarea { font-size: 12px !important; min-height: 40px !important; padding: 4px 8px !important; }
    .result-box .text { font-size: 12px !important; }
}
</style>
...
<div class="app-header">
    ...
    <p class="subtitle">Voice &amp; Text · 8 Languages</p>
</div>
""", unsafe_allow_html=True)
...
if not st.session_state.deepl_api_key or not st.session_state.cohere_api_key:
    st.markdown('<div class="glass-card" style="text-align:center; font-size:12px;">🔐 Connect API Keys</div>', unsafe_allow_html=True)
    ...
    with col2:
        if not st.session_state.cohere_api_key:
            cohere_input = st.text_input("Cohere Key", type="password", placeholder="abcd-1234")
            if cohere_input:
...
input_text = st.text_area("", height=50, placeholder="اكتب أو الصق النص هنا...", value=st.session_state.input_text, key="input_text_area")
...
st.markdown("""
<div style="text-align:center; padding: 0.3rem 0; color:rgba(100,130,170,0.25); font-size:7px; letter-spacing:0.1em; text-transform:uppercase;">
    HN TRANSLATOR · Voice Suite
</div>
""", unsafe_allow_html=True)
