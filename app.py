...
LN=list(LANGS.keys()); LN_NO_AUTO=[k for k in LN if k!="Auto-Detect"]
...
# ═══════════════════════════════════════════════════════════
_DB=os.path.join(tempfile.gettempdir(),"hn_v5.db")
...
def _mpush(e):
    ...
    st.session_state["_mem"].insert(0,e); st.session_state["_mem"]=st.session_state["_mem"][:150]
def save_tr(orig,trans,emo,src,tgt,eng=""):
    ...
    try:
        c=_db()
        c.execute('''INSERT INTO history(original,translated,emotion,source_lang,
                     target_lang,engine,timestamp)VALUES(?,?,?,?,?,?,?)''',
...
def get_hist(n=60):
    ...
    try:
        ...
        rows=c.execute('''SELECT original,translated,emotion,source_lang,
                          target_lang,engine,timestamp FROM history
        ...
        sigs={(e.get("original","")[:80],e.get("time","")) for e in mem}
        return (mem+[d for d in db_items if (d["original"][:80],d["time"]) not in sigs])[:n]
    except: return mem[:n]
def clr_hist():
    st.session_state["_mem"]=[]
    try: c=_db(); c.execute("DELETE FROM history"); c.commit(); c.close()
    except: pass
def del_item(orig,ts):
    st.session_state["_mem"]=[e for e in st.session_state.get("_mem",[])
                               if not(e.get("original")==orig and e.get("time")==ts)]
    try:
        c=_db(); c.execute("DELETE FROM history WHERE original=? AND timestamp=?",(orig,ts))
        c.commit(); c.close()
    except: pass
init_db()

# ═══════════════════════════════════════════════════════════
#  API KEYS — مخفية تماماً، تُقرأ من secrets.toml فقط
# ═══════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════
#  GROQ LLM
#  نموذجان: FAST (8b) للترجمة والتصحيح  — SMART (70b) للمحادثة المعقدة
#  FAST: 14,400 طلب/يوم  |  SMART: 100 طلب/يوم  — نحافظ على الحصة
# ═══════════════════════════════════════════════════════════
_M_FAST  = "llama-3.1-8b-instant"      # سريع، حصة كبيرة
_M_SMART = "llama-3.3-70b-versatile"   # ذكي، حصة محدودة

def groq_llm(prompt, system="", max_tokens=600, model=None):
    k=_gk()
    ...
    try:
        r=requests.post("https://api.groq.com/openai/v1/chat/completions",
            ...
            json={"model":m,"messages":msgs,"max_tokens":max_tokens,"temperature":0.35},
            timeout=25)
...
# ═══════════════════════════════════════════════════════════
#  TRANSLATION
#  ترتيب الأولوية: Google أولاً (سريع جداً) → DeepL → AI
#  الذكاء الاصطناعي فقط للمحادثة والترجمة الثقافية العميقة
# ═══════════════════════════════════════════════════════════
_TR_SYS=(
    ...
    "2. Proverbs/idioms: find the CULTURAL EQUIVALENT in the target language\n"
    "3. Dialects (Arabic: مصري/خليجي/شامي/مغربي/عراقي; all world dialects): "
    "understand first, then translate naturally into standard language\n"
    "4. Preserve register: formal=formal, casual=casual, poetic=poetic\n"
    "5. Return ONLY the translation — no explanations, no quotes, no preamble"
)

def fast_translate(text, tgt, src="Auto-Detect"):
    """Google / DeepL — سريع بدون AI"""
    ...
    if _dk():
        ...
        if info.get("d"):
            ...
            try:
                r=requests.post(ep,headers={"Authorization":f"DeepL-Auth-Key {_dk()}"},
...
def smart_translate(text, tgt, src="Auto-Detect"):
    """
    للترجمة الفورية: Google أولاً (أسرع + بلا حصة)
    للنصوص الصعبة (أمثال/عامية): Groq 8b-fast
    """
    if not text or not text.strip(): return None,"no text"
    # DeepL إن وُجد — جودة عالية
    if _dk():
        info=LANGS.get(tgt,{})
        if info.get("d"):
            ep=("https://api-free.deepl.com/v2/translate" if _dk().endswith(":fx")
    ...
    # Google — سريع وبلا حصة (أولوية للترجمة الفورية)
    src_g=LANGS.get(src,{}).get("g","auto"); tgt_g=LANGS.get(tgt,{}).get("g","en")
    try:
        res=GoogleTranslator(source=src_g or "auto",target=tgt_g).translate(text)
        if res: return res,"Google"
    except: pass
    # Groq fast — للنصوص القصيرة الصعبة فقط
    if _gk() and len(text)<=800:
        r,_=groq_llm(f"Translate from {src} to {tgt}:\n{text}",
                     system=_TR_SYS,max_tokens=600,model=_M_FAST)
        if r: return r,"AI ✦"
    return None,"فشلت الترجمة"

# ═══════════════════════════════════════════════════════════
#  هل تقصد؟ — اقتراحات الإملاء الفوري
# ═══════════════════════════════════════════════════════════
@st.cache_data(show_spinner=False, ttl=300)
def spell_suggest(text, src_lang):
    """
    فحص إملائي سريع بـ 8b-instant — يُعيد list من الاقتراحات
    مُخزَّن في الكاش لـ 5 دقائق لتجنب استنزاف الحصة
    """
    if not _gk() or not text or len(text.strip())<4: return []
    r,_=groq_llm(
        f'Check this {src_lang} text for spelling/grammar errors: "{text}"\n'
        'Return a JSON array of corrections ONLY (no markdown). '
        'Format: [{"wrong":"...","correct":"..."}] '
        'Max 3 corrections. If no errors return: []',
        max_tokens=150, model=_M_FAST)
    if not r: return []
    try:
        clean=re.sub(r'```json|```','',r).strip()
        items=json.loads(clean)
        return [(x["wrong"],x["correct"]) for x in items
...
# ═══════════════════════════════════════════════════════════
#  AI CHAT ASSISTANT
#  يستخدم 70b للمحادثة الذكية، مع fallback لـ 8b
# ═══════════════════════════════════════════════════════════
_AI_SYS=(
    "أنت مساعد ترجمة ذكي محترف مدمج في HN Translator.\n\n"
    "قدراتك الكاملة:\n"
    ...
    "▸ تغيير الأسلوب: رسمي، أدبي، تقني، محادثاتي، شعري\n"
    "▸ الإجابة عن أي سؤال لغوي أو ثقافي\n"
    "▸ تحويل العامية للفصحى والعكس\n\n"
    "قواعد ذهبية:\n"
    "• افهم القصد وراء الكلمات دائماً\n"
    "• الأمثال والتعابير: أوجد المقابل الثقافي، لا الترجمة الحرفية\n"
    "• ردودك مباشرة وواضحة — بلا مقدمات زائدة\n"
    "• إذا طُلبت ترجمة، أعطها فوراً بلا تأخير"
)

def ai_chat(user_msg, source_text, current_trans, src_lang, tgt_lang, history):
    if not _gk(): return None,"لا يوجد مفتاح Groq في secrets.toml"
    ctx=(f"السياق الحالي:\n"
         f"- النص الأصلي ({src_lang}): {source_text[:500] if source_text else 'لا يوجد'}\n"
         f"- الترجمة الحالية ({tgt_lang}): {current_trans[:350] if current_trans else 'لا يوجد'}\n"
         f"- اتجاه الترجمة: {src_lang} → {tgt_lang}\n\n"
         f"طلب المستخدم: {user_msg}")
    msgs=[{"role":"system","content":_AI_SYS}]
    for h in history[-8:]: msgs.append({"role":h["role"],"content":h["content"]})
    msgs.append({"role":"user","content":ctx})
    k=_gk()
    # نجرب 70b أولاً، ثم 8b إن فشل (حصة 70b محدودة)
    for model in [_M_SMART, _M_FAST]:
        try:
            ...
            if r.status_code==429:
                continue  # جرّب النموذج الأصغر
            return None,f"Groq {r.status_code}"
        except Exception as e:
            return None,str(e)
    return None,"تجاوزت الحد — انتظر دقيقة وأعد المحاولة"

def _chat_err(e):
    s=str(e or "")
    if "429" in s: return "⚠️ تجاوزت الحد المجاني — انتظر دقيقة وأعد المحاولة"
    if "401" in s: return "❌ مفتاح Groq غير صالح — تحقق من GROQ_API_KEY في secrets.toml"
    if "timeout" in s.lower(): return "⚠️ انتهت مهلة الاتصال — حاول مجدداً"
    return f"⚠️ {s or 'خطأ غير معروف'}"

# ═══════════════════════════════════════════════════════════
#  TTS / EMOTION / LANG DETECT / DOMAIN
# ═══════════════════════════════════════════════════════════
def make_tts(text,lang="en"):
    if not text or not text.strip(): return None
    try:
        buf=io.BytesIO(); gTTS(text=text[:500],lang=lang,slow=False).write_to_fp(buf)
...
def quick_domain(text):
    tl=text.lower()
    return [d for d,s in sorted([(d,sum(tl.count(k) for k in v["kw"])) for d,v in _DOM.items()],
                                  key=lambda x:-x[1]) if s>0][:2]

# ═══════════════════════════════════════════════════════════
#  FILE / OCR / STT / GROUP / SUBTITLES
# ═══════════════════════════════════════════════════════════
        ...
        try:
            ...
            with pdfplumber.open(io.BytesIO(fb)) as pdf:
                for pg in pdf.pages:
                    t=pg.extract_text()
                    if t: txt+=t+"\n"
            return (txt.strip(),None) if txt.strip() else (None,"No text")
        except Exception as e: return None,str(e)
    if ext==".docx":
...
def ocr_image(img):
    if not _gk(): return None,"أضف GROQ_API_KEY في secrets.toml"
    try:
                ...
                json={"model":model,"temperature":0,"max_tokens":2048,
                      "messages":[{"role":"user","content":[
                          ...
                          {"type":"text","text":"Extract ALL text exactly as written. "
                           "Preserve original language. Return ONLY raw text."}]}]},
                timeout=30)
            if r.status_code==200:
...
def groq_stt(audio,lang="auto",verbose=False):
    k=_gk()
    if not k: return None,"No Groq key"
    lc=_wl(lang)
    ...
    try:
        r=requests.post("https://api.groq.com/openai/v1/audio/transcriptions",
            headers={"Authorization":f"Bearer {k}"},files=files,timeout=60)
        if r.status_code==200:
            data=r.json()
...
def video_to_srt(audio_bytes,tgt):
    ...
    for i,s in enumerate(segs,1):
        ...
        orig_b.append(b); tr,_=smart_translate(b["text"],tgt)
...
def _css(t):
    ...
    if dk:
        ac="#4ECBA0"
        bg="linear-gradient(135deg,#07071a 0%,#0c1525 55%,#070f1a 100%)"
        card="rgba(78,203,160,.08)"; brd="rgba(78,203,160,.2)"
        txt="#F0F4FF"; sub="rgba(78,203,160,.78)"; sbg="rgba(7,7,26,.98)"
        ib="#0f1827"; it="#F0F4FF"; ph="#3a7a60"
        cu="rgba(78,203,160,.12)"; ca="rgba(255,255,255,.04)"
        sb_grad="linear-gradient(180deg,#07071a 0%,#0a1020 100%)"
        sb_head="rgba(78,203,160,.06)"; sb_acc="#4ECBA0"
    else:
        ac="#1a9e70"; bg="#EFF3F8"
        card="rgba(26,158,112,.07)"; brd="rgba(26,158,112,.22)"
        txt="#111827"; sub="rgba(26,158,112,.82)"; sbg="rgba(255,255,255,.99)"
        ib="#FFFFFF"; it="#111827"; ph="rgba(26,158,112,.4)"
        cu="rgba(26,158,112,.1)"; ca="rgba(0,0,0,.04)"
        sb_grad="linear-gradient(180deg,#f0f7f4 0%,#e8f5f0 100%)"
        sb_head="rgba(26,158,112,.06)"; sb_acc="#1a9e70"
...
textarea::placeholder{{color:{ph} !important;opacity:.6 !important;
...
/* ── هل تقصد ── */
.spell-row{{display:flex;align-items:center;gap:.4rem;flex-wrap:wrap;
  padding:.45rem .8rem;margin:.25rem 0;
  background:rgba(255,180,0,.08);border:1px solid rgba(255,180,0,.25);
  border-radius:10px;font-size:13px}}
.spell-label{{color:rgba(255,180,0,.9);font-weight:700;font-size:11px;white-space:nowrap}}
.spell-pill{{background:rgba(255,180,0,.12);border:1px solid rgba(255,180,0,.35);
  border-radius:20px;padding:3px 12px;font-size:12px;font-weight:600;
  color:{txt};cursor:pointer;transition:all .15s;white-space:nowrap}}
.spell-pill:hover{{background:rgba(255,180,0,.25);border-color:rgba(255,180,0,.6)}}
/* ── Chat ── */
...
[data-testid="stSidebar"]{{
  background:{sb_grad} !important;
  border-right:1px solid {brd} !important;
  padding-top:0 !important}}
/* شريط علوي في السايدبار */
.sb-header{{
  background:{sb_head};
  border-bottom:1px solid {brd};
  padding:.75rem .9rem .6rem;
  margin-bottom:.3rem}}
.sb-logo{{display:flex;align-items:center;gap:.5rem;margin-bottom:.4rem}}
.sb-logo-icon{{font-size:20px;line-height:1}}
.sb-logo-text{{font-size:11px;font-weight:800;letter-spacing:.15em;
  color:{sb_acc};text-transform:uppercase}}
.sb-logo-ver{{font-size:8px;color:{sub};opacity:.55;margin-top:1px}}
.sb-status{{display:flex;align-items:center;gap:.35rem;
  font-size:10px;font-weight:600;color:{sub};opacity:.75}}
.sb-dot{{width:6px;height:6px;border-radius:50%;
  background:{ac};box-shadow:0 0 6px {ac}80;flex-shrink:0}}
.sb-dot.off{{background:#555;box-shadow:none}}
/* تاريخ العنوان */
.sb-hist-title{{
  display:flex;align-items:center;justify-content:space-between;
  padding:.4rem .9rem .2rem;
  font-size:9px;font-weight:800;letter-spacing:.15em;
  text-transform:uppercase;color:{sub};opacity:.6}}
/* بطاقات التاريخ */
.hist-card{{
  background:linear-gradient(135deg,{card} 0%,rgba(78,203,160,.04) 100%);
  border:1px solid {brd};border-radius:14px;
  padding:.7rem .9rem;margin:.0 .5rem .45rem;
...
[data-testid="stSidebar"] button[kind="secondary"]{{
  ...
  box-shadow:none !important;opacity:.55}}
[data-testid="stSidebar"] button[kind="secondary"]:hover{{
  background:{card} !important;opacity:1 !important;
  border-color:#e55 !important;color:#e55 !important}}
/* ── Audio / Tabs / Selects ── */
...
<div class="br-row"><span class="dot">◆</span>
...
# ═══════════════════════════════════════════════════════════
#  SIDEBAR — تصميم مميز، بلا مفاتيح
# ═══════════════════════════════════════════════════════════
with st.sidebar:
    # ── شريط علوي بالسايدبار ──────────────────────────────
    ai_on = bool(_gk())
    st.markdown(f"""<div class="sb-header">
<div class="sb-logo">
  <span class="sb-logo-icon">🌐</span>
  <div>
    <div class="sb-logo-text">HN Translator</div>
    <div class="sb-logo-ver">v5 · Smart AI</div>
  </div>
</div>
<div class="sb-status">
  <span class="sb-dot {'on' if ai_on else 'off'}"></span>
  <span>{'الذكاء الاصطناعي نشط' if ai_on else 'Google Translate فقط'}</span>
</div>
</div>""", unsafe_allow_html=True)

    # ── زر تبديل المظهر ───────────────────────────────────
    lbl = "☀️ الوضع الفاتح" if st.session_state.theme=="dark" else "🌙 الوضع الداكن"
    if st.button(lbl, use_container_width=True, key="_theme_btn"):
    ...
    # ── سجل المحفوظات ─────────────────────────────────────
    hist = get_hist(60)
    st.markdown(f"""<div class="sb-hist-title">
  <span>🕘 سجل المحفوظات</span>
  <span style="font-size:8px;opacity:.5">{len(hist)} ترجمة</span>
</div>""", unsafe_allow_html=True)
    ...
    if hist:
        ...
        for i, it in enumerate(hist):
            ...
            orig=orig_f[:52]+("…" if len(orig_f)>52 else "")
            trans=trans_f[:52]+("…" if len(trans_f)>52 else "")
            eng=it.get("engine",""); tgt=it.get("target_lang","")
            ts=it.get("time",""); emo=it.get("emotion","")
            num=f"#{len(hist)-i:02d}"
            if "AI" in eng:   bc="#4ECBA0"; el="✦ AI"
            elif "DeepL" in eng: bc="#6ea8fe"; el="⬡ DeepL"
            else:             bc="#f4a261"; el="◎ Google"
            ei=emo.split()[0] if emo else "·"
  ...
  <div class="hist-divider">
    ...
    <span class="hist-divider-icon">{ei} ↓</span>
    <div class="hist-divider-line"></div>
  </div>
  <div class="hist-trans">{trans}</div>
  <div class="hist-foot">
    <span class="hist-badge" style="background:{bc}18;color:{bc};border-color:{bc}40">{el}</span>
    <span class="hist-meta">{tgt} · {ts}</span>
  </div>
</div>""", unsafe_allow_html=True)
            with col_d:
                st.markdown("<div style='height:18px'></div>", unsafe_allow_html=True)
                if st.button("✕", key=f"_d_{i}", help="حذف"):
                    del_item(orig_f, ts); st.rerun()
    else:
        st.markdown("""<div style="display:flex;flex-direction:column;align-items:center;
justify-content:center;padding:2.5rem 1rem;opacity:.25;gap:.5rem">
...
with tab2:
    ...
    if input_text and len(input_text.strip())>=2:
        ...
        if ck!=st.session_state.get("_tc_key"):
            # Google أولاً للسرعة — AI للمحادثة
            res=fast_translate(input_text.strip(),tgt_name,src_name)
        ...
        if auto_trans and ck!=st.session_state.get("_saved_ck"):
            save_tr(input_text.strip(),auto_trans,emotion(input_text.strip()),
    ...
    with col_t:
        ...
        if auto_trans:
            ...
            with cc:
                if input_text.strip():
                    st.markdown(
                        f"<div style='font-size:14px;padding-top:.4rem'>{emotion(input_text)}</div>",
                        unsafe_allow_html=True)
        else:
            st.markdown(
    ...
    # ── هل تقصد؟ (تصحيح إملائي فوري) ───────────────────────
    if input_text and len(input_text.strip())>=5 and _gk():
        sp_key=f"_sp_{input_text.strip()[:60]}"
        if sp_key not in st.session_state:
            st.session_state[sp_key]=spell_suggest(input_text.strip(),src_name)
        suggestions=st.session_state.get(sp_key,[])
        if suggestions:
            pills="".join(
                f'<span class="spell-pill" onclick="void(0)">'
                f'<b>{w}</b> ← <span style="text-decoration:line-through;opacity:.6">{x}</span>'
                f'</span>'
                for x,w in suggestions)
            st.markdown(
                f'<div class="spell-row">'
                f'<span class="spell-label">🔤 هل تقصد؟</span>{pills}</div>',
                unsafe_allow_html=True)
            # أزرار تطبيق التصحيح
            for idx,(wrong,correct) in enumerate(suggestions):
                if st.button(f'✅ استبدل "{wrong}" بـ "{correct}"',
                             key=f"_sapp_{idx}",use_container_width=False):
                    st.session_state.input_text=input_text.replace(wrong,correct,1)
                    st.rerun()
    ...
    pair_idx=0; i=0
    while i<len(chat):
        msg=chat[i]; is_u=msg["role"]=="user"; cls="msg-u" if is_u else "msg-a"
        if is_u and i+1<len(chat) and chat[i+1]["role"]=="assistant":
            st.markdown(f'<div class="{cls}">{msg["content"]}</div>',
                        unsafe_allow_html=True)
            col_m,col_d=st.columns([20,1])
            with col_m:
                st.markdown(f'<div class="msg-a">{chat[i+1]["content"]}</div>',
                            unsafe_allow_html=True)
            with col_d:
                st.markdown("<div style='height:6px'></div>",unsafe_allow_html=True)
                if st.button("✕",key=f"_cd_{pair_idx}",help="حذف"):
                    del chat[i:i+2]; st.rerun()
            i+=2
        else:
            col_m,col_d=st.columns([20,1])
            with col_m:
    ...
    if q:=st.chat_input("اسأل المساعد الذكي…",key="_ci"):
        ...
        reply=ans if ans else _chat_err(err)
    ...
    if uf:
        ...
        if err: st.error(f"❌ {err}")
            ...
            with st.spinner("⏳ ترجمة الملف..."):
                ...
                parts=[]; bar2=st.progress(0)
                for j,ch in enumerate(chunks):
                    r,_=fast_translate(ch,tgt_name,src_name)
                    parts.append(r or ch); bar2.progress((j+1)/len(chunks))
                bar2.empty()
            full_tr="\n".join(parts); emo=emotion(txt[:400])
            st.markdown(f"""<div class="card">
...
with tab5:
    st.markdown("---")
    if not _gk():
        st.info("💡 هذه الميزة تتطلب GROQ_API_KEY في secrets.toml")
    else:
        av_file=st.file_uploader("🎬 ارفع ملف صوت/فيديو",
            type=["wav","mp3","mp4","m4a","ogg","webm"],
            key="_avf",label_visibility="visible")
        if av_file:
            if st.button("🚀 استخراج وترجمة",use_container_width=True,key="_av_btn"):
