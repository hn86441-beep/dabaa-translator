...
import requests, os, json, tempfile, io, base64, sqlite3, re, traceback, time, urllib.parse
...
# ═══════════════════════════════════════════════════════════
#  WHATSAPP ERROR ALERTS (عبر CallMeBot المجاني)
#  الخطوات لتفعيلها:
#  1) أضف رقم واتساب +34 644 59 71 67 إلى جهات اتصالك
#  2) أرسل له رسالة نصها بالضبط: I allow callmebot to send me messages
#  3) سيصلك API Key على واتساب — ضَعه في secrets.toml باسم CALLMEBOT_APIKEY
# ═══════════════════════════════════════════════════════════
_WA_PHONE = "201152582350"   # الرقم بصيغة دولية (مصر +20)
try:    _WA_APIKEY = (st.secrets.get("CALLMEBOT_APIKEY", "") or "").strip()
except: _WA_APIKEY = ""
_WA_COOLDOWN_FILE = os.path.join(tempfile.gettempdir(), "hn_wa_cooldown.json")
_WA_COOLDOWN_SEC  = 600  # لا يُرسل نفس نوع الخطأ أكثر من مرة كل 10 دقائق

def _wa_should_send(err_key):
    try:
        data = {}
        if os.path.exists(_WA_COOLDOWN_FILE):
            with open(_WA_COOLDOWN_FILE, "r") as f:
                data = json.load(f)
        last = data.get(err_key, 0)
        if time.time() - last < _WA_COOLDOWN_SEC:
            return False
        data[err_key] = time.time()
        with open(_WA_COOLDOWN_FILE, "w") as f:
            json.dump(data, f)
        return True
    except:
        return True
...
def notify_whatsapp(message, err_key=None):
    """يرسل تنبيه واتساب عند حدوث خطأ في الموقع أو في مفتاح GROQ"""
    if not _WA_APIKEY or not _WA_PHONE:
        return
    key = err_key or message[:60]
    if not _wa_should_send(key):
        return
    try:
        text = urllib.parse.quote(f"🚨 HN TRANSLATOR\n{message[:500]}")
        requests.get(
            "https://api.callmebot.com/whatsapp.php",
            params={"phone": _WA_PHONE, "text": text, "apikey": _WA_APIKEY},
            timeout=8)
    except:
        pass
...
def groq_llm(prompt, system="", max_tokens=700, fast=False):
    ...
    try:
        ...
        if r.status_code == 401:
            notify_whatsapp("مفتاح GROQ غير صالح أو منتهي (401 Unauthorized)", err_key="groq_401")
        elif r.status_code == 429:
            notify_whatsapp("تجاوز الحد المسموح لطلبات GROQ (429 Too Many Requests)", err_key="groq_429")
        else:
            notify_whatsapp(f"خطأ من GROQ — كود {r.status_code}", err_key=f"groq_{r.status_code}")
    except Exception as e:
        notify_whatsapp(f"تعذر الاتصال بـ GROQ: {e}", err_key="groq_conn")
    return None
...
try:
    ...
    notify_whatsapp(f"خطأ غير متوقع في الموقع:\n{_app_err}\n\n{_tb[-400:]}", err_key='site_error')
    st.error("⚠️ حدث خطأ غير متوقع في التطبيق. تم إرسال تنبيه تلقائي. حاول تحديث الصفحة.")
    with st.expander("تفاصيل الخطأ (للمطوّر)"):
        st.code(_tb)
